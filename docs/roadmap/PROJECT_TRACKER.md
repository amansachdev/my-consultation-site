...
- **Payments/video:** Razorpay Standard Checkout order creation, client checkout, signature verification, and payment-linked booking flow are implemented behind `PAYMENTS_ENABLED`; invoices, refunds, payouts, and provider-based video remain future work
...
| M4 | Booking, Calendar & Availability | MVP | Done | Guest and signed-in booking requests, Cosmos persistence, Resend owner notification, immediate Google Meet event/link creation, shared admin-managed 30-minute recurring availability, reserved-slot filtering, blocked dates/slots, one-booking-per-slot reservation, idempotent retries, rate limiting, timezone handling, and timed Join access are implemented | Pricing, refund policy, and follow-up rules remain business/clinical policy work; appointment lifecycle enhancements can be added separately |
| M5 | Payments, Invoicing & Payouts | MVP | In Progress | Razorpay Standard Checkout order creation, UPI/cards/net banking modal, server-side signature verification, and payment-linked booking confirmation; invoices/receipts, refunds, cancellation handling, commission calculation, and clinician payout ledger remain out of scope | Q-BIZ-04 (refund policy), Q-BIZ-05 (commission), Q-BIZ-06 (GST), live Razorpay environment configuration |
...
| M8 | Prescription & Documentation | MVP | In Progress | Clinician/admin-gated local prescription PDF generation with patient/medicine validation and a low-cost Cosmos audit record is implemented; digital signature/approval, consultation summary, follow-up scheduling, and document management are not | Q-CLIN-06 (controlled substances), Q-LEGAL-02 (consent), Q-LEGAL-06 (prescription record consent/retention), Q-CLIN-07 (follow-up window) |
...
| M11 | Security, Privacy & Compliance | MVP | In Progress | Server-side RBAC, consent versioning, prescription audit logging, security headers, and draft privacy/terms pages are implemented; encryption verification, secure document storage, backups, data-retention/deletion, breach-response runbook, and pen-test plan remain | Q-LEGAL-02 (final policy/consent wording), Q-LEGAL-03 (retention), Q-LEGAL-05 (DPDP), Q-TECH-07 (data localization), Q-TECH-08 (backup targets) |
| M12 | DevOps, Infra & Observability | MVP | In Progress | CI/CD (GitHub Actions → Azure Static Web Apps), PR lint/build checks, Azure Functions deployment, environment-secret wiring, and local MSW development mode are implemented; managed backups, structured monitoring, alerting, uptime monitoring, and staging remain | Q-TECH-07 (cloud region: Static Web Apps hosted in East Asia; custom domain + CDN), Q-TECH-08 (RTO/RPO) |
...
| M2: Booking & payments | M4-M5 | A patient can pay ₹500 before the booking request is submitted and receive booking/Meet confirmation; invoicing and refunds remain out of scope | In Progress |
...
| 2026-09-15 | Integrated ₹500 Razorpay Standard Checkout into the booking flow: server order creation, frontend payment modal, HMAC signature verification, payment-linked booking records, and payment failure/cancellation handling; invoices, GST, refunds, and payouts remain out of scope | Agent |
| 2026-09-19 | L1: Removed GitHub Pages workflow and related config (homepage, deploy scripts, gh-pages dependency, VITE_BASE_URL override); Azure (`antaran.online`) confirmed as canonical production host | Agent |
| 2026-09-19 | M1: Added unit test infrastructure and 116 tests — extracted `scoreAssessment` into `api/src/scoring.js`, extracted `verifySignature`/`validateBooking`/`cleanText`/`cleanPayment` into `api/src/validation.js`, created `vitest.config.unit.js`, added `npm test` script and CI gate; covers PHQ-9/GAD-7 scoring boundaries, client–server cross-checks, HMAC verification, and booking validation | Agent |
| 2026-09-19 | M4 follow-up: Optimized large assets (`consultation-hero.png` -> `.jpg`, `dr medha profile.png` -> `.jpg`), reducing bundle size and improving LCP | Agent |
