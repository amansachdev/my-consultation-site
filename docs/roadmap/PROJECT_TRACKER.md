...
- **Backend:** Azure Functions API and Cosmos DB persistence configured in Azure
...
- **Design system:** Antaran logo palette approved; tokens seeded; legacy landing page still uses original color aliases
...
|----|--------|-------|--------|------------------|---------------------------|
| M0 | Foundation & Design System | MVP | 🟡 In Progress | `AGENTS.md`, design tokens, Tailwind wiring, folder structure, repo conventions, CI lint/build | Q-BIZ-01 (final brand/domain) resolved: domain is `antaran.online`; remaining work includes applying tokens consistently to the legacy landing page |
...
| M8 | Prescription & Documentation | MVP | 🟡 In Progress | Clinician-only local prescription PDF generation is implemented; digital signature/approval flow, consultation summary, follow-up scheduling, and document management remain | Q-CLIN-06 (controlled substances), Q-LEGAL-02 (consent), Q-CLIN-07 (follow-up window) |
...
| M3: Consultation experience | M6-M8 | Clinician and patient can join video, produce notes & prescription | 🟡 In Progress |
...
| ID | Resolution |
|----|------------|
| ✅ B-01 | Cosmos DB, Firebase token verification, Resend, and Google Calendar production settings are configured in the `antaran-site` Azure Static Web App. |
